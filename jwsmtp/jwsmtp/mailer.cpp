...
void mailer::operator()()
{
    ...
    for(std::vector<SOCKADDR_IN>::const_iterator address = adds.begin(); address < adds.end(); ++address) {
        ScopedSocket s;
        ...
        int len1;
        if(!s.Recv(len1, buff, buffsize -1, 0)) {
        ...
        if(!s.Send(len1, commandline.c_str(), commandline.length(), 0)) {
        ...
        if(returnstring.substr(0,3) != OK) {
            ...
            if(returnstring.substr(0,3) != OK) {
                ...
                if(s.Send(len1, "QUIT\r\n", 6, 0)) {
                    char dummy[buffsize];
                    s.Recv(len1, dummy, buffsize -1, 0);
                }
                s.Closesocket();
        ...
        if(!s.Send(len1, &smtpheader[0], smtpheader.size(), 0)) {
...
std::vector<char> mailer::makesmtpmessage() const
{
    ...
    ///////////////////////////////////////////////////////////////////////////
    // add the subject
    headerline = "Subject: =?" + charset + "?B?" + base64encode(subject, false) + "?=\r\n\r\n";
    ret.insert(ret.end(), headerline.begin(), headerline.end());
    ...
    if(MIME) {
        ...
        if(!messageHTML.size()) {
            ...
            headerline += "Content-type: text/plain; charset=";
            headerline += charset;
            ...
            headerline += "Content-type: text/html; charset=";
            headerline += charset;
...
bool mailer::gethostaddresses(std::vector<SOCKADDR_IN>& adds)
{
    ...
    int ret;
    if(!s.Send(ret, (char*)dns, dnspos, 0)) {
        returnstring = "451 Requested action aborted: server seems to have disconnected.";
        s.Closesocket(); // clean up
        return false;
    }
    if(s.Recv(ret, (char*)dns, 512, 0)) {
        s.Closesocket();
...
bool mailer::authenticate(const std::string& servergreeting, ScopedSocket& s)
{
    ...
    if(type == LOGIN) {
        greeting = "auth " + at + "\r\n";
        if(!s.Send(len, greeting.c_str(), greeting.length(), 0)) {
            returnstring = "554 send failure: \"auth " + at + "\"";
            return false;
        }
        if(!s.Recv(len, buff, buffsize, 0)) {
        ...
        std::string rsphead = returnstring.substr(0,16);
        if(rsphead != "334 VXNlcm5hbWU6" && rsphead != "334 dXNlcm5hbWU6") {
        ...
        rsphead = returnstring.substr(0,16);
        if(rsphead != "334 UGFzc3dvcmQ6" && rsphead != "334 cGFzc3dvcmQ6") {
