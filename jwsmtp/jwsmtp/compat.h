...
public:
    ...
    SOCK_TYPE getSock() const
    {
        return _sock;
    }
    bool Connect(const SOCKADDR_IN& addr);
    bool Create(int domain, int type, int protocol);
    bool Send(int &CharsSent, const char *msg, size_t len, int flags);
    bool Recv(int &CharsRecv, char *buf, size_t len, int flags);
    void Closesocket();
private:
    ScopedSocket(const ScopedSocket& s);
    ScopedSocket&  operator= (const ScopedSocket& s);
    SOCK_TYPE _sock;
};
